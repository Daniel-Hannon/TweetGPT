...
def get_trend_tweets(driver, trend):
   ...
   if trend.__contains__("Promoted"):
      return True
   #put the trend name in the search box
   searchbox.send_keys(trend)
   searchbox.send_keys(u'\ue007')

   # add the trend to the database
   cur.execute("INSERT INTO trends(trending_topic_name) VALUES (?)", (trend,))
   trend_id = cur.execute("SELECT (id) FROM trends WHERE trending_topic_name = (?)", (trend,)).fetchone()[0]
...
if __name__ == "__main__":
   trend_names = get_trend_tweets(chrome_options)
   print(trend_names)
   con.close()